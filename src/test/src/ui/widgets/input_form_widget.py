...
class InputForm(QVBoxLayout):
    def __init__(self, label_text: str, options: List[str]):
        super().__init__()
        self._init_components(label_text, options)
        self._setup_layout()
        self._connect_signals()

    def _init_components(self, label_text: str, options: List[str]):
        """
        初始化自动选择相关组件
        # -------------- 要实现的内容 start -------------
        # 布局的定义
    ...
    def _connect_signals(self):
        """连接所有信号"""
        self._radio_need_option.toggled.connect(self._update_visibility)
